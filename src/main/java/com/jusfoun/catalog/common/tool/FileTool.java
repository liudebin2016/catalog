...
public class FileTool extends org.apache.commons.io.FileUtils {
		...
		if (fileDir.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				if (files[i].isFile()) {
					...
					// 如果是目录，则递归调用
					FileTool.zipDirectoryToZipFile(dirPath, files[i],
							zouts);
				}
			}
	...
	public static String path(String path){
		String p = StringTool.replace(path, "\\", "/");
		p = StringTool.join(StringTool.split(p, "/"), "/");
		if (!StringTool.startsWithAny(p, "/") && StringTool.startsWithAny(path, "\\", "/")){
			p += "/";
		}
		if (!StringTool.endsWithAny(p, "/") && StringTool.endsWithAny(path, "\\", "/")){
			p = p + "/";
		}
		return p;
	}
